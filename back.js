...
function addSigns(signs) {
  if (resultJS.innerHTML.substr(0) == "") {
    ...
    resultJS.innerHTML = resultJS.innerHTML.substring(
      0,
      resultJS.innerHTML.length - 3
    );
    resultJS.innerHTML += signs;
  } else if (resultJS.innerHTML.substr(-1) == ".") {
    resultJS.innerHTML = resultJS.innerHTML.substring(
      0,
      resultJS.innerHTML.length - 1
    );
    resultJS.innerHTML += signs;
  } else {
    resultJS.innerHTML += signs;
...
function addPoint() {
  if (resultJS.innerHTML.substr(0) == "") {
    ...
    for (var cont = resultJS.innerHTML.length; cont >= 0; cont--) {
      if (
        resultJS.innerHTML.substr(cont - resultJS.innerHTML.length) == " " ||
...
function calculate() {
  ...
  if (resultJS.innerHTML.includes(".")) {
    space = false;
  }
...
bMultiplication.addEventListener("click", function () {
  addSigns(" * ");
});
bDivsion.addEventListener("click", function () {
  addSigns(" / ");
});
